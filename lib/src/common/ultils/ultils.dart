...
class Utils {
  ...
  static Future<bool> existsFile(String path) async {
    return await File('$_currentPath/$path').existsSync();
  }

  static void createFile(String path) async {
    await File('$_currentPath/$path').createSync(recursive: true);
  }

  static void writeFile(String path, String text) async {
    await File('$_currentPath/$path').writeAsStringSync(text);
  }

  static Future<bool> existsScreen(String screen) async {
    var presentationDir = Directory('$_currentPath/lib/presentation');
